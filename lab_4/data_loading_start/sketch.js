...
function setup() {
    ...
    let quizMarks = studentData.getColumn('quiz').map(Number);    // Оценки за тест
    let projectMarks = studentData.getColumn('project').map(Number); // Оценки за проект
    let finalMarks = studentData.getColumn('finalMark').map(Number); 
    let examMarks = studentData.getColumn('exam').map(Number);      

    // средние значения 
    let avgQuiz = average(quizMarks);
    let avgProject = average(projectMarks);
    let avgFinal = average(finalMarks);
    let avgExam = average(examMarks); 

    myBar = new Bar(10, 10, 700, 700);
    myBar.setupAxis(0, 4, 0, 100, 1, 10); 

    let data = [avgQuiz, avgProject, avgFinal, avgExam]; 
    let labels = ["Quiz", "Project", "Final", "Exam"]; 
    let colours = ['red', 'blue', 'green', 'orange']; 

    myBar.addBars(data, labels, colours); 
}
...
function Bar(x, y, width, height) {
    ...
    this.setupAxis = function(xMin, xMax, yMin, yMax, xTickSpacing, yTickSpacing) {
        ...
        line(this.plotX, this.plotY + this.plotHeight, this.plotX + this.plotWidth, this.plotY + this.plotHeight); // Ось X
        line(this.plotX, this.plotY, this.plotX, this.plotY + this.plotHeight); // Ось Y

        //ось X
        for (let i = xMin; i <= xMax; i++) {
            let tickX = map(i, xMin, xMax, this.plotX, this.plotWidth + this.plotX);
            line(tickX, this.plotY + this.plotHeight, tickX, this.plotY + this.plotHeight + 5); // Метка
            textAlign(CENTER);
            text(i, tickX, this.plotY + this.plotHeight + 15); // Значения
        }

        //ось Y
        for (let j = yMin; j <= yMax; j += yTickSpacing) {
            let tickY = map(j, yMin, yMax, this.plotHeight + this.plotY, this.plotY);
            line(this.plotX - 5, tickY, this.plotX, tickY); // Метка
            textAlign(RIGHT);
            text(j, this.plotX - 15, tickY); // Значения
        }
    };

    this.addBars = function(dataArray, labels, colours) {
        for (let i = 0; i < dataArray.length; i++) {
            let barHeight = dataArray[i];
            let colour = colours[i];
            this.addBar(i + 1, barHeight, 30, colour, labels[i]); 
        }
    };

    this.addBar = function(xValue, yValue, barWidth, colour, label) {
        var pointX = map(xValue, 0, 4, this.plotX, this.plotWidth + this.plotX);
        var pointY = map(yValue, 0, 100, this.plotHeight + this.plotY, this.plotY);
        
        stroke(0); 
        strokeWeight(2);
        fill(colour); 
        rect(pointX - barWidth / 2, pointY, barWidth, this.plotHeight - pointY); 

        //подпись под столбцом
        noStroke();
        fill(0);
        textAlign(CENTER);
        text(label, pointX, this.plotY + this.plotHeight + 20);
    };
}